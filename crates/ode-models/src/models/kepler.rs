...
//! pendulums ([`super::gl4`]), so flow_inv = one step with −dt is exact.
...
///
/// let mut sys = KeplerSystem::new(perihelion_state(0.5), 0.01, KeplerObservation::Q1);
/// sys.forward();                    // one step: t^0 → t^1
/// // sys.states[n] = (q₁, q₂, p₁, p₂) at t^n
/// ```
pub struct KeplerSystem {
    ...
    /// Trajectory so far: `states[n]` = (q₁, q₂, p₁, p₂) at t^n. Holds the
    /// initial condition after [`new`]; each [`forward`] call appends one
    /// state. The last entry is the current state.
...
impl KeplerSystem {
    ...
    pub fn new(x0: [f64; DIM], dt: f64, observation: KeplerObservation) -> Self {
        Self::with_params(KeplerParams::default(), x0, dt, observation)
    }
    ...
    /// arbitrary input state (angular difference for the bearing).
...
impl Model<DIM> for KeplerSystem {
    ...
    fn forward(&mut self) {
        KeplerSystem::forward(self);
    }

    fn discrepancy(&self, xi: &[f64]) -> f64 {
        KeplerSystem::discrepancy(self, xi)
    }
...
mod tests {
    ...
    fn run(params: KeplerParams, x0: [f64; DIM], dt: f64, steps: usize) -> KeplerSystem {
        let mut sys = KeplerSystem::with_params(params, x0, dt, KeplerObservation::Q1);
        for _ in 0..steps {
            sys.forward();
        }
        sys
    ...
    fn energy_is_conserved() {
        for params in [KeplerParams::default(), ODD_PARAMS] {
            ...
            let sys = run(params, x0, 0.005, 1260); // t ≈ 2π
    ...
    fn angular_momentum_is_conserved_exactly() {
        ...
        let sys = run(ODD_PARAMS, x0, 0.02, 500);
    ...
    fn jacobian_matches_finite_differences() {
        let sys = KeplerSystem::with_params(ODD_PARAMS, perihelion_state(0.0), 0.01, KeplerObservation::Q1);
    ...
    fn flow_inv_inverts_flow() {
        let x0 = perihelion_state(0.5);
        let sys = KeplerSystem::new(x0, 0.01, KeplerObservation::Range);
        let y = sys.flow(&x0);
    ...
    fn flow_is_defined_at_the_origin() {
        let sys = KeplerSystem::new(perihelion_state(0.5), 0.01, KeplerObservation::Q1);
