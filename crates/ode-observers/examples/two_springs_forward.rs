//! Forward solve only: the two-mass spring-chain reference trajectory, no
//! filter (same setup as the `two_springs` example).
//!
//! Use this to inspect the range each state component actually visits before
//! choosing the filter-domain intervals of the `two_springs` example. Prints
//! the per-component min/max and writes the trajectory for
//! `visualize_model.py`.
//!
//! Outputs go to examples/output/two_springs_forward/.
...
fn main() {
    ...
    save_forward::<M, _>(&sys, &reference, "examples/output/two_springs_forward");
}
